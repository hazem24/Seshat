         ...
         Class Retweet extends TwModel
        {
            ...
            */
            public function startProcess(array $data = []){
                    ...
                    $sendProcess =  $this->retweet($screenName,$email,$password,$retweet_to);
                    if(is_array($sendProcess) &&  array_key_exists('followSend',$sendProcess)){
                        ...
                        $this->log(false,'لم يستطيع عمل ريتويت.',$screenName,$this->userId,0);
                    }                    
                        sleep(1);                       
                        return true;
            }

            private function retweet(string $screenName , string $email , string $password , string $retweet_to){
                    /**
                    *1-Login To Twitter 
                    ...
                    */
                    ...
                    if(is_array($loginPage) && array_key_exists('loggedIn',$loginPage)){
                            ...
                            $send_follow = $this->send_follow($auth_code,$retweet_to , $email);
                             ...
                             $response =  ['followNotSend'=>true];
                                ...
                                $send_follow = $this->send_follow($loginPage['auth_code'],$retweet_to , $email);
                    }
            ...
            private function send_follow(string $auth_code,string $retweet_to , string $email){
                    $ch= curl_init();
                    curl_setopt_array($ch, array(
                    CURLOPT_URL => TW_MOBILE."/statuses"."/$retweet_to"."/retweet",
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_POST => true,
                    CURLOPT_POSTFIELDS => "authenticity_token=$auth_code&tweet_id=$retweet_to&_method=POST&return_url=/#tweet_$retweet_to&commit=Retweet",
